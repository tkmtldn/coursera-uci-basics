...
The program should print “Not Found!” otherwise. The program should not be case-sensitive, so it does
...
Submit your source code for the program, “uci_findian.go”.
 */

 func main(){
  fmt.Println("Let's find Ian! Please enter a string: ")
  inn := bufio.NewReader(os.Stdin)
  s, _ := inn.ReadString('\n')
  s = s[:len(s)-1]
  s = strings.ToLower(s)
  if strings.Contains(s, "a") && strings.HasPrefix(s, "i") && strings.HasSuffix(s, "n") {
   fmt.Println("Found!") } else { fmt.Println("Not found!")}
 }