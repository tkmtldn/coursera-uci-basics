...
Submit your source code for the program, “uci_slice.go”.
 */
 ...
 func main() {
 	...
 	for {
		...
		if s != "X" {
			i, _ := strconv.Atoi(s)
			this_slice = append(this_slice, i)
			sort.Ints(this_slice)
			fmt.Println(this_slice)
			 } else {
			fmt.Print("Goodbye!", this_slice)
			break }
	}
 }
